...
class ChatClient:
    def __init__(self, root):
        ...
        self.disconnect_button = Button(root, text='Отключиться', command=self.connect_to_server)
    ...
    def send_message(self, event=None):
        ...
        if message:
            ...
            try:
                ...
                self.add_message(f'Ошибка при отправке: {str(e)}')
    ...
    def connect_to_server(self):
        ...
        try:
            ...
            messagebox.showerror('Error', f'Не удалось подключиться: {e}')
    def receive_messages(self):
        while self.connected:
            try:
                ...
                if self.connected:
                    self.add_message('Ошибка при получении сообщений')
                    self.disconnect_from_server()
                break
    ...
    def disconnect_from_server(self):
        if self.client_socket:
            ...
            self.add_message('Отключено от сервера')
